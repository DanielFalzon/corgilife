...
compileFinal "
	...
	[CL_bmoola,CL_cmoola,owner player,player,profileNameSteam,getPlayerUID player,playerSide] remoteExecCall [""CL_fnc_adminInfo"",_ret];
";
...
compileFinal "
	...
	if (player isEqualTo _unit && !(_vehicle in CL_vehicles)) then {
...
compileFinal "
	...
	if ((call CL_adminlevel) < 1) exitWith {hint ""You are not an admin!"";};
	private [""_msg"",""_to""];
	ctrlShow[3020,false];
	_msg = ctrlText 3003;
	_to = call compile format [""%1"",(lbData[3004,(lbCurSel 3004)])];
...
compileFinal "
	...
	switch (_type) do {
		...
		case 2 : {
			if ((call CL_adminlevel) < 1) exitWith {};
			private [""_message"",""_loc"",""_unit""];
			_loc = _this select 3;
			_unit = _this select 4;
		...
		case 3 : {
			...
			systemChat _message;
			if ((call CL_adminlevel) > 0) then {systemChat _admin;};
			player say3D ""alert"";
		};
...
if (!CL_markers) then {
	...
	for ""_i"" from 0 to 1 step 0 do {
		if (!CL_markers) exitWith {};
	{
	if ((vehicle _x isKindOf ""LandVehicle"") || (vehicle _x isKindOf ""Air"") || (vehicle _x isKindOf ""Ship"")) then {
		if (count(crew vehicle _x) > 0) then {
		{
			if (!(_x in CL_markers_Vehicles) && (alive _x) && (getPlayerUID _x != """")) then {
			...
			CL_markers_Vehicles pushBack _x;
			[_x, _Markers, _Vehicle, _TypeVehicle] spawn {
				...
				_Marker = _this select 1;
								...
								for ""_i"" from 0 to 1 step 0 do {
									if (!CL_markers && !(alive _PlayersOrVehicles) && (vehicle _PlayersOrVehicles == _PlayersOrVehicles) && (getPlayerUID _PlayersOrVehicles != """")) exitWith {};
				...
				if (_PlayersOrVehicles in CL_markers_Vehicles) then {
									CL_markers_Vehicles deleteAt (CL_markers_Vehicles find _PlayersOrVehicles);
				};
				true;
			};
			};
		...
		if (!(_x in CL_markers_Players) && (vehicle _x == _x) && (getPlayerUID _x != """")) then {
		...
		CL_markers_Players pushBack _x;
		[_x, _Markers] spawn {
			...
			deleteMarkerLocal _Marker;
			if (_PlayersOrVehicles in CL_markers_Players) then {
							CL_markers_Players deleteAt (CL_markers_Players find _PlayersOrVehicles);
			};
			true;
		};
		};
