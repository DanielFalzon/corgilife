...
if (!(count _queryResult isEqualTo 0)) exitWith {
	[1,"There is already a gang created with that name please pick another name."] remoteExecCall ["CL_fnc_broadcast",_ownerID];
	CL_action_gangInUse = nil;
	_ownerID publicVariableClient "CL_action_gangInUse";
};
...
if (!(count _queryResult isEqualTo 0)) exitWith {
	[1,"You are currently already active in a gang, please leave the gang first."] remoteExecCall ["CL_fnc_broadcast",_ownerID];
	CL_action_gangInUse = nil;
	_ownerID publicVariableClient "CL_action_gangInUse";
};

// Factions
_query = format ["SELECT id FROM gangs WHERE faction='%1' AND active='1'",_faction];
...
if (!(count _queryResult isEqualTo 0)) then {
	_query = format ["UPDATE gangs SET active='1', owner='%1',members='%2',faction='%3' WHERE id='%4'",_uid,_gangMembers,_faction,(_queryResult select 0)];
} else {
	_query = format ["INSERT INTO gangs (owner, name, members, faction) VALUES('%1','%2','%3','%4')",_uid,_gangName,_gangMembers,_faction];
};
...
_group setVariable ["gang_faction",_faction,true];
[_group] remoteExecCall ["CL_fnc_gangCreated",_ownerID];
