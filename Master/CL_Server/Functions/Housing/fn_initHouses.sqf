...
private _blacklistedHouses = "count (getArray (_x >> 'garageBlacklists')) > 0" configClasses (missionconfigFile >> "Housing" >> "CL");
private _blacklistedGarages = "count (getArray (_x >> 'garageBlacklists')) > 0" configClasses (missionconfigFile >> "Garages" >> "CL");
...
for "_i" from 0 to count(_blacklistedHouses)-1 do {
	...
	_positions = getArray(missionConfigFile >> "Housing" >> "CL" >> _className >> "garageBlacklists");
	{
...
for "_i" from 0 to count(_blacklistedGarages)-1 do {
	...
	_positions = getArray(missionConfigFile >> "Garages" >> "CL" >> _className >> "garageBlacklists");
	{
		_obj = nearestObject [_x,_className];
		if (isNull _obj) then {
			_obj setVariable ["blacklistedGarage",true,true];
