...
_resourceItems = CL_Settings(getArray,"save_vehicle_items");

if (isNull _vehicle || isNull _unit) exitWith {CL_impound_inuse = false; (owner _unit) publicVariableClient "CL_impound_inuse";CL_garage_store = false;(owner _unit) publicVariableClient "CL_garage_store";}; //Bad data passed.
...
//Save damage & fuel
if (CL_Settings(getNumber,"save_vehicle_damage") isEqualTo 1) then {
...
if (count _vInfo isEqualTo 0) exitWith {
	[1,"STR_Garage_Store_NotPersistent",true] remoteExecCall ["CL_fnc_broadcast",(owner _unit)];
	CL_garage_store = false;
	(owner _unit) publicVariableClient "CL_garage_store";
};

if (_uid != getPlayerUID _unit) exitWith {
	[1,"STR_Garage_Store_NoOwnership",true] remoteExecCall ["CL_fnc_broadcast",(owner _unit)];
	CL_garage_store = false;
	(owner _unit) publicVariableClient "CL_garage_store";
};
...
if (CL_Settings(getNumber,"save_vehicle_virtualItems") isEqualTo 1) then {
	if (CL_Settings(getNumber,"save_vehicle_illegal") isEqualTo 1) then {
		...
		if (_blacklist) then {
			[_uid, _profileName, "481"] remoteExecCall["CL_fnc_wantedAdd", RSERV];
...
CL_garage_store = false;
(owner _unit) publicVariableClient "CL_garage_store";
[1,_storetext] remoteExecCall ["CL_fnc_broadcast",(owner _unit)];