...
if ((_vInfo select 5) isEqualTo 0) exitWith {
	...
	[1,"STR_Garage_SQLError_Destroyed",true,[_vInfo select 2]] remoteExecCall ["CL_fnc_broadcast",_unit];
};

if ((_vInfo select 6) isEqualTo 1) exitWith {
	serv_sv_use deleteAt _servIndex;
	[1,"STR_Garage_SQLError_Active",true,[_vInfo select 2]] remoteExecCall ["CL_fnc_broadcast",_unit];
};
...
if (count _nearVehicles > 0) exitWith {
	serv_sv_use deleteAt _servIndex;
	[_price,_unit_return] remoteExecCall ["CL_fnc_garageRefund",_unit];
	[1,"STR_Garage_SpawnPointError",true] remoteExecCall ["CL_fnc_broadcast",_unit];
};
...
[_vehicle] remoteExecCall ["CL_fnc_addVehicle2Chain",_unit];
[_pid,_side,_vehicle,1] call TON_fnc_keyManagement;
_vehicle lock 2;
//Reskin the vehicle
[_vehicle,(_vInfo select 8)] remoteExecCall ["CL_fnc_colorVehicle",_unit];
...
if (CL_Settings(getNumber,"save_vehicle_virtualItems") isEqualTo 1) then {
	...
	if (_wasIllegal) then {
		...
		[1,"STR_NOTF_BlackListedVehicle",true,[_location,_name]] remoteExecCall ["CL_fnc_broadcast",west];
...
if (CL_Settings(getNumber,"save_vehicle_fuel") isEqualTo 1) then {
	_vehicle setFuel (_vInfo select 11);
	}else{
	_vehicle setFuel 1;
};

if (count _gear > 0 && (CL_Settings(getNumber,"save_vehicle_inventory") isEqualTo 1)) then {
...
if (count _damage > 0 && (CL_Settings(getNumber,"save_vehicle_damage") isEqualTo 1)) then {
...
if ((_vInfo select 1) isEqualTo "civ" && (_vInfo select 2) isEqualTo "B_Heli_Light_01_F" && !((_vInfo select 8) isEqualTo 13)) then {
	[_vehicle,"civ_littlebird",true] remoteExecCall ["CL_fnc_vehicleAnimate",_unit];
};
...
if ((_vInfo select 1) isEqualTo "cop" && ((_vInfo select 2)) in ["C_Offroad_01_F","B_MRAP_01_F","C_SUV_01_F","C_Hatchback_01_sport_F","B_Heli_Light_01_F","B_Heli_Transport_01_F"]) then {
	[_vehicle,"cop_offroad",true] remoteExecCall ["CL_fnc_vehicleAnimate",_unit];
};

if ((_vInfo select 1) isEqualTo "med" && (_vInfo select 2) isEqualTo "C_Offroad_01_F") then {
	[_vehicle,"med_offroad",true] remoteExecCall ["CL_fnc_vehicleAnimate",_unit];
};

[1,_spawntext] remoteExecCall ["CL_fnc_broadcast",_unit];
serv_sv_use deleteAt _servIndex;
