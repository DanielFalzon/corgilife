...
if (CL_bmoola < _value) exitWith {hint "You don't have enough cash in your bank account!";};

if (_type isEqualTo 0) then
	{
		_diff = [_item,_amount,CL_carryWeight,CL_maxWeight] call CL_fnc_calWeightDiff;
		...
		if (_type isEqualTo 1)then
			{ if (_amount > 1) then
				{
					_iCheck = [_item,_amount] call CL_fnc_vAH_checkItem;
				};
			};
		};
...
switch (_type) do {
	case 0: {[true,_item,_diff] call CL_fnc_handleInv;};
	case 1: {
	_tamount = _amount + 1;
...
[0,[_item,_value,name _unit]] remoteExec ["CL_fnc_vAH_reciever",_caller];
[1,[_caller,_value]] remoteExec ["CL_fnc_vAH_reciever",_unit];
} else {
[3,_id,_value,_seller] remoteExec ["TON_fnc_vAH_update",2];
[0,[_item,_value,_sellerName]] remoteExec ["CL_fnc_vAH_reciever",_caller];
};
closeDialog 0;
