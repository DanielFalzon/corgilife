...
_bad = false;
_realName = "";
switch (FETCH_CONST(CL_donorlevel)) do {
	case 1: {_able = 2};
	case 2: {_able = 3};
	case 3: {_able = 4};
	case 4: {_able = 5};
...
if (_price < 1) exitWith {hint "Invalid value. In this life, nothing is for free son."};
if (_price > 10000000) exitWith {hint "Invalid value. Must be lower than 10 mill. Scrooge."};
...
switch (_type) do
{
	...
	case 1: {if (_quantity > 1) then
			...
			_realName = ([_classname] call CL_fnc_fetchCfgDetails) select 1;
			};
};
...
if(_action) then {
		if(CL_bmoola < _tax) exitWith {hint "You don't have enough money in the bank to pay the tax";_bad = true;};

	switch (_type) do {
		case 0: {if(!([false,_classname,_quantity] call CL_fnc_handleInv)) then {hint "You dont have that many!";_bad = true;};};
		case 1: {
			_tquantity = _quantity +1;
			for [{_i=1}, {_i<_tquantity}, {_i=_i+1}] do
			{
				[_classname,false] spawn CL_fnc_handleItem;
			};
		};
	};
	if (_bad) exitWith {closeDialog 0;};
