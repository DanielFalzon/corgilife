	...
	} else {
{
	...
	switch (_type) do {
		case "imenu": {
			_itemDisplayName = ([_item] call CL_fnc_fetchCfgDetails) select 1;
			...
			if (_price isEqualTo 0) then {_priceDisplay = ""} else {_priceDisplay = format[": Auction Price $%1",[_price]call CL_fnc_numberText];};
			};
		case "ymenu": {
