...
_arr_resource = [_shortname] call CL_fnc_marketGetRow;
_arr_price = [_shortname] call CL_fnc_marketGetPriceRow;
...
[_shortname, [_shortname,_price,_globalprice,-(_modifier)], false ] call CL_fnc_marketSetPriceRow; //dont broadcast!
...
if (!_israw) then {
	{
		if ((count _x) isEqualTo 2) then {
			...
			[_x select 0, _relamount, true, false] call CL_fnc_marketBuy; //Make prices higher, no broadcast!
		} else {
			_relamount = -(_relamount);
			[_x select 0, _relamount, true, false] call CL_fnc_marketSell; //Make prices higher, no broadcast!
			};
		} else {
