...
if (!isClass(missionConfigFile >> "VirtualShops" >> CL_shop_type)) exitWith {closeDialog 0; hint localize "STR_NOTF_ConfigDoesNotExist";}; //Make sure the entry exists..
ctrlSetText[2403,localize (M_CONFIG(getText,"VirtualShops",CL_shop_type,"name"))];
_shopItems = M_CONFIG(getArray,"VirtualShops",CL_shop_type,"items");

{
	_displayName = M_CONFIG(getText,"VirtualItems",_x,"displayName");
	_price = M_CONFIG(getNumber,"VirtualItems",_x,"buyPrice");

	_marketprice = [_x] call CL_fnc_marketGetBuyPrice;
	if (_marketprice != -1) then {
			_price = _marketprice;
		};

	if (!(_price isEqualTo -1)) then {
		_item_list lbAdd format ["%1  ($%2)",(localize _displayName),[_price] call CL_fnc_numberText];
...
[_shopItems] spawn CL_fnc_marketShortView;
