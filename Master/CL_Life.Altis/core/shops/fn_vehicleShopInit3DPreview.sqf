...
*/
// Creating a camera.
CL_preview_3D_vehicle_cam = "camera" camCreate ([[5000, 5000, 0]] call CL_fnc_searchPosEmpty);
CL_preview_3D_vehicle_cam cameraEffect ["Internal", "BACK"];
CL_preview_3D_vehicle_cam camSetFocus [-1, -1];
showCinemaBorder false;
CL_preview_3D_vehicle_cam camCommit 0;
...
0 spawn
{
	...
	for "_i" from 0 to 1 step 0 do {
		if (isNull CL_preview_3D_vehicle_cam) exitWith {};
		private ["_object","_distanceCam","_azimuthCam"];

		// Waiting for a view object.
		waitUntil {!isNull CL_preview_3D_vehicle_object};

		_object = CL_preview_3D_vehicle_object;
		...
		CL_preview_3D_vehicle_cam camSetTarget _object;
		CL_preview_3D_vehicle_cam camSetPos (_object modelToWorld [_distanceCam * sin _azimuthCam, _distanceCam * cos _azimuthCam, _distanceCam * 0.33]);
		CL_preview_3D_vehicle_cam camCommit 0;

		// Rotation around the object.
		for "_i" from 0 to 1 step 0 do {
			if (!(CL_preview_3D_vehicle_object isEqualTo _object)) exitWith {};
			_azimuthCam = _azimuthCam + 1.00;

			CL_preview_3D_vehicle_cam camSetPos (_object modelToWorld [_distanceCam * sin _azimuthCam, _distanceCam * cos _azimuthCam, _distanceCam * 0.33]);
			CL_preview_3D_vehicle_cam camCommit 0.05;
