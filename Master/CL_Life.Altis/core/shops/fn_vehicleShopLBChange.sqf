...
private ["_className","_classNamein","_initalPrice","_buyMultiplier","_rentMultiplier","_vehicleInfo","_colorArray","_ctrl","_trunkSpace","_maxspeed","_horsepower","_passengerseats","_fuel","_armor"];
...
_initalPrice = M_CONFIG(getNumber,"CLCfgVehicles",_classNamein,"price");
...
[_className] call CL_fnc_vehicleShop3DPreview;
...
(CONTROL(2300,2303)) ctrlSetStructuredText parseText format [
	...
	[round(_initalPrice * _rentMultiplier)] call CL_fnc_numberText,
	[round(_initalPrice * _buyMultiplier)] call CL_fnc_numberText,
	_maxspeed,
	_horsepower,
	_passengerseats,
...
_colorArray = M_CONFIG(getArray,"CLCfgVehicles",_classNamein,"textures");

{
	_flag = (_x select 1);
	_textureName = (_x select 0);
	if ((CL_veh_shop select 2) isEqualTo _flag) then {
		_x params ["_texture"];
		private _toShow = [_x] call CL_fnc_levelCheck;
		if (_toShow) then {
