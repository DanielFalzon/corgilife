...
if !([_conditions] call CL_fnc_levelCheck) exitWith {hint localize "STR_Shop_Veh_NotAllowed"};

if (CL_Settings(getNumber,"vehicleShop_3D") isEqualTo 1) then {
createDialog "CL_Vehicle_Shop_v2_3D";
} else {
createDialog "CL_Vehicle_Shop_v2";
};

CL_veh_shop = [_shop,_spawnpoints,_shopFlag,_disableBuy]; //Store it so so other parts of the system can access it.
