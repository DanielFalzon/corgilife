...
if (!isNull _house) then {
	_message = 1;
	if (([player] call CL_fnc_playerInBuilding) && {([_container] call CL_fnc_playerInBuilding)}) then {
		_message = 2;
		if ((_house in CL_vehicles) && !(isNil {_house getVariable "house_owner"})) then {
			_message = 3;
			if (!_isFloating) then {
				_message = 4;
				_containers = _house getVariable ["containers",[]];
				_houseCfg = [(typeOf _house)] call CL_fnc_houseConfig;
				if (_houseCfg isEqualTo []) exitWith {};
				if (count _containers < (_houseCfg select 1)) then {
					_isPlaced = true;
					if (CL_HC_isActive) then {
						[_uid,_container] remoteExec ["HC_fnc_addContainer",HC_CL];
					} else {
...
[true,_type,1] call CL_fnc_handleInv;
