...
CL_action_inUse = true;//Lock out other actions during processing.
...
if (_filter) exitWith {CL_action_inUse = false;};

_itemInfo = [_materialsRequired,_materialsGiven,_noLicenseCost,(localize format["%1",_text])];
if (count _itemInfo isEqualTo 0) exitWith {CL_action_inUse = false;};
...
_exit = false;
if (count _oldItem isEqualTo 0) exitWith {CL_action_inUse = false;};
...
if (_exit) exitWith {CL_is_processing = false; hint localize "STR_NOTF_NotEnoughItemProcess"; CL_action_inUse = false;};

	if (_vendor isEqualTo prison_processing_tablet) then {
		_hasLicense = true;
	};
	if(_vendor in [uranium1_process_npc,uranium2_process_npc,uranium3_process_npc,uranium4_process_npc]) then {
		_hasLicense = LICENSE_VALUE("uraniumA","civ");
	}else {
...
_oldItemWeight = 0;
{
	_weight = ([_x select 0] call CL_fnc_itemWeight) * (_x select 1);
...
if (_newItemWeight > _oldItemWeight) then {
	...
	_freeSpace = CL_maxWeight - CL_carryWeight;
...
if (_exit) exitWith {hint localize "STR_Process_Weight"; CL_is_processing = false; CL_action_inUse = false;};
...
if (_hasLicense) then {
	...
	if (player distance _vendor > 10) exitWith {hint localize "STR_Process_Stay"; "progressBar" cutText ["","PLAIN"]; CL_is_processing = false; CL_action_inUse = false;};

	{
		[false,(_x select 0),((_x select 1)*(_minimumConversions))] call CL_fnc_handleInv;
	} count _oldItem;

	{
		[true,(_x select 0),((_x select 1)*(_minimumConversions))] call CL_fnc_handleInv;
	} count _newItem;
	...
	if (CL_cmoola < _cost) exitWith {hint format[localize "STR_Process_License",[_cost] call CL_fnc_numberText]; "progressBar" cutText ["","PLAIN"]; CL_is_processing = false; CL_action_inUse = false;};
