...
for "_i" from 0 to 1 step 0 do {
	...
	if (CL_istazed) exitWith {_hideout setVariable ["inCapture",false,true];}; //Tazed
	if (CL_isknocked) exitWith {_hideout setVariable ["inCapture",false,true];}; //Knocked
	if (CL_interrupted) exitWith {_hideout setVariable ["inCapture",false,true];};
};

//Kill the UI display and check for various states
"progressBar" cutText ["","PLAIN"];
player playActionNow "stop";
if (!alive player || CL_istazed || CL_isknocked) exitWith {CL_action_inUse = false;_hideout setVariable ["inCapture",false,true];};
if (player getVariable ["restrained",false]) exitWith {CL_action_inUse = false;_hideout setVariable ["inCapture",false,true];};
if (CL_interrupted) exitWith {CL_interrupted = false; titleText[localize "STR_GNOTF_CaptureCancel","PLAIN"]; CL_action_inUse = false;_hideout setVariable ["inCapture",false,true];};
...
[[0,1],"STR_GNOTF_CaptureSuccess",true,[name player,(group player) getVariable "gang_name"]] remoteExecCall ["CL_fnc_broadcast",RCLIENT];
_hideout setVariable ["inCapture",false,true];
_hideout setVariable ["gangOwner",group player,true];
["ClaimHideout"] spawn mav_ttm_fnc_addExp;
