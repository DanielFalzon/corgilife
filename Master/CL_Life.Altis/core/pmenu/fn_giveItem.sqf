...
ctrlShow[2002,false];
if ((lbCurSel 2023) isEqualTo -1) exitWith {hint "No one was selected!";
ctrlShow[2002,true];};
...
if (!([false,_item,(parseNumber _val)] call CL_fnc_handleInv)) exitWith {hint "Couldn't give that much of that item, maybe you don't have that amount?"; ctrlShow[2002,true];};

[_unit,_val,_item,player] remoteExecCall ["CL_fnc_receiveItem",_unit];
