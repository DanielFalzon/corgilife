...
private _crimes = CL_Settings(getArray,"crimes");
{
...
if (FETCH_CONST(CL_coplevel) < 3) then {
	ctrlShow[2405,false];
};

if (CL_HC_isActive) then {
	[player] remoteExec ["HC_fnc_wantedFetch",HC_CL];
} else {
	[player] remoteExec ["CL_fnc_wantedFetch",RSERV];
};
