...
_details = [_item] call CL_fnc_fetchCfgDetails;
	...
	switch (_details select 6) do {
		...
		case "CfgVehicles": {
			...
			if (!isNil "_items") then {
				{[_x,true,true,false,true] call CL_fnc_handleItem; } forEach _items;
			};
		};
				...
				switch (_details select 5) do {
						...
						if (_ispack) then {
							...
							if (_override) then{
								...
								if (vest player isEqualTo _item && {!_preview}) then {
									...
									if (!isNil "_items") then {
										{[_x,true,false,false,true] spawn CL_fnc_handleItem;} forEach _items;
									};
								};
							};
						};
					...
					case 201: {
						if (_ispack) then {
							...
							private "_type";
							_type = [_item,201] call CL_fnc_accType;
							if (_ongun) then {
								switch (_type) do {
					...
					case 301: {
						if (_ispack) then {
							...
							private "_type";
							_type = [_item,301] call CL_fnc_accType;
					...
					case 101:{
						if (_ispack) then {
							...
							private "_type";
							_type = [_item,101] call CL_fnc_accType;
