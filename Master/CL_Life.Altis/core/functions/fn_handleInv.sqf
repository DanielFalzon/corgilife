...
if (_math) then {
	_diff = [_item,_num,CL_carryWeight,CL_maxWeight] call CL_fnc_calWeightDiff;
	_num = _diff;
	if (_num <= 0) exitWith {false};
};
_weight = ([_item] call CL_fnc_itemWeight) * _num;
_value = ITEM_VALUE(_item);

if (_math) then {
	//Lets add!
	if ((CL_carryWeight + _weight) <= CL_maxWeight) then {
		missionNamespace setVariable [_var,(_value + _num)];

		if ((missionNamespace getVariable _var) > _value) then {
			CL_carryWeight = CL_carryWeight + _weight;
			_return = true;
		} else {
			_return = false;
		};
