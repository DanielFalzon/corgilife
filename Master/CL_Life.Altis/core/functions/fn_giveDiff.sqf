...
if (_bool) then {
	if ([true,_item,(parseNumber _val)] call CL_fnc_handleInv) then {
		...
		[format ["%1 (%2) attempted to give %3 (%4) %5 %6.",profileName,getPlayerUID player,_from getVariable ["realname",name _from],getPlayerUID _from,_val,(localize _type)],"ItemLog"] remoteExecCall ["A3Log",2];
	};
} else {
	if ([true,_item,(parseNumber _val)] call CL_fnc_handleInv) then {
