...
private _veh_data = [_vehicle] call CL_fnc_vehicleWeight;
...
[_vehicle] call CL_fnc_vehInventory;
CL_trunk_vehicle = _vehicle;
...
_vehicle spawn {
	...
	if (_this isKindOf "Box_IND_Grenades_F" || _this isKindOf "B_supplyCrate_F") then {
		...
		if (CL_HC_isActive) then {
			[_this] remoteExecCall ["HC_fnc_updateHouseTrunk",HC_CL];
		} else {
...
if (CL_Settings(getNumber,"save_vehicle_virtualItems") isEqualTo 1) then {
	_vehicle spawn {
		...
		if ((_this isKindOf "Car") || (_this isKindOf "Air") || (_this isKindOf "Ship")) then {
			...
			if (CL_HC_isActive) then {
				[_this,2] remoteExecCall ["HC_fnc_vehicleUpdate",HC_CL];
			} else {
