...
if (isNil {_vehicle getVariable "fuelTank"}) then{
	_fuelSpace = getNumber(missionConfigFile >> "CLCfgVehicles" >> (typeOf _vehicle) >> "vFuelSpace");
	_fuelState = 0;
...
if (_another)exitWith{titleText[localize "STR_FuelTank_AnotherInUse","PLAIN"];CL_action_inUse = false;};
...
if (_fuelFeedState <= 0) exitWith {titleText [localize "STR_FuelTank_FeedFull","PLAIN"]; CL_action_inUse = false;};
...
if (_distance < 1000) exitWith {titleText [localize "STR_FuelTank_PipeLine","PLAIN"]; CL_action_inUse = false;};

_pricem = getNumber(missionConfigFile >> "CL_Settings" >> "fuelTank_winMultiplier");
_price = floor((((floor(_shortest / 100) * 100) / 1337) * _pricem) * 100) / 100;
_win = 0;

_vehicle setVariable ["fuelTankWork",true,true];
_vehicle remoteExec ["CL_fnc_soundDevice",-2];
CL_action_inUse = false;
