...
if (!(nearestObject [player, "House"] in CL_vehicles)) exitWith {hint localize "STR_ISTR_Box_NotinHouse";};

CL_container_active = true;
closeDialog 0;
...
CL_container_activeObj = _object;
_attachPos = [0.16, 3, ((boundingBoxReal _object) select 1) select 2];
[_object] remoteExecCall ["CL_fnc_simDisable",RANY];
