...
private _level = CL_currentExpLevel;
private _levelConfigBase = _baseConfig >> "Levels";
private _currentExpLevelConfig = _levelConfigBase >> format["Level_%1", CL_currentExpLevel];
...
private _iteratedExp = CL_currentExp;
...
while {_dif > 0} do {
    ...
    if !(_level isEqualTo CL_currentExpLevel) then {
...
CL_currentExp = CL_currentExp + _expGained;
CL_currentPerkPoints = CL_currentPerkPoints + _gainedPerkPoints;
