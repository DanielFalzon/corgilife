...
try {
	...
	// -- Initialized?
	if !(missionNamespace getVariable ["CL_perksInitialized", false]) exitWith {};
	...
	_updateGUI = {
		private _currentLevelConfig = missionConfigFile >> "Maverick_TTM" >> "Levels" >> format["Level_%1", CL_currentExpLevel];
		private _nextLevelConfig = missionConfigFile >> "Maverick_TTM" >> "Levels" >> format["Level_%1", CL_currentExpLevel + 1];
		...
		private _currentLevelRelativeExp = (CL_currentExp - _currentLevelBottomExp);
		private _currentLevelProgress = if (_currentLevelRelativeExp != 0) then {((CL_currentExp - _currentLevelBottomExp) / (_currentLevelTopExp - _currentLevelBottomExp))} else {0};

		// -- Display data
		_ctrlStrucTextCurrentRank ctrlSetStructuredText parseText format ["<t size='3' shadow='0' align='center'>%1</t>", CL_currentExpLevel];
		_ctrlTextRankName ctrlSetText getText (_currentLevelConfig >> "displayName");

		if (isClass _nextLevelConfig) then {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1/%2, Perk Points Available: %3", CL_currentExp - _currentLevelBottomExp, _currentLevelTopExp - _currentLevelBottomExp, CL_currentPerkPoints];
			_ctrlProgress progressSetPosition _currentLevelProgress;
		} else {
			_ctrlTextStatusInfo ctrlSetText format ["XP: %1, Maximum Level, Perk Points Available: %2", CL_currentExp - _currentLevelBottomExp, CL_currentPerkPoints];
	...
	switch (_mode) do
	{
		...
		{
			// -- Not loaded in yet?
			if (!CL_perksInitialized) exitWith {
			...
			[CL_currentExpPerks, []] call _generateTreeView;
		...
		case "onTreeSelectionChanged":
		{
			...
			private _hasRequiredLevel = ((getNumber (_selectedPerkConfig >> "requiredLevel")) <= CL_currentExpLevel);
			private _limitedToFactions = getArray (_selectedPerkConfig >> "limitToSides");
			private _hasParents = if (_requiredPerk != "") then {[CL_currentExpPerks, getText (_selectedPerkConfig >> "requiredPerk")] call mav_ttm_fnc_hasPerk;} else {true};

			CL_currentExpPerks call _iterate;

			if (((count _limitedToFactions) == 0) || {(str playerSide) in _limitedToFactions}) then {
				if (_hasRequiredLevel) then {
					if (_hasParents) then {
						if (!_ownsPerk) then {
							if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > CL_currentPerkPoints) then {
		...
		case "onButtonPurchaseClick":
		{
			...
			if ((getNumber (_selectedPerkConfig >> "requiredPerkPoints")) > CL_currentPerkPoints) exitWith {
			...
			CL_currentPerkPoints = CL_currentPerkPoints - (getNumber (_selectedPerkConfig >> "requiredPerkPoints"));
