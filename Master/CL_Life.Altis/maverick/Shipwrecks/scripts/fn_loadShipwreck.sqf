...
if(hasInterface) exitWith {
    ...
    player addAction[getText(missionConfigFile >> "Maverick_Shipwrecks" >> "ShipLootText"),{
        [] spawn {
            ...
            {
                if([true,_x,1] call CL_fnc_handleInv) then {
    ...
    },"",0,false,false,"",'!CL_wrecklooting && ((count nearestObjects[player,["Land_Wreck_Traw_F"], 20]) != 0) && (count(((nearestObjects[player,["Land_Wreck_Traw_F"], 20]) select 0) getVariable["wreckloot", []]) != 0)'];
};
