...
if (getNumber(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Info" >> "music") == 1) then {
	0 fadeMusic (getNumber(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Info" >> "musicVolume"));
	playMusic (getText(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Info" >> "musicName"));
	0 fadeSound 0;
};

// Load first and start the loop
mav_introcam_scripthandle = [] spawn {
	_time = getNumber(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Info" >> "secondsPerCam");
	_firstData = (getArray(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Positions" >> "data")) select 0;
	...
	while {true} do {
		// Load new position
		_newData = [];
		if ((getNumber(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Info" >> "selectRandom")) == 1) then {
			_newData = (getArray(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Positions" >> "data")) call BIS_fnc_selectRandom;
		} else {
			...
			if (isNil {(getArray(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Positions" >> "data")) select mav_introcam_lastindex}) then {
				mav_introcam_lastindex = 0;
			};

			_newData = (getArray(MissionconfigFile >> "Maverick_IntroCam_CL" >> "Positions" >> "data")) select mav_introcam_lastindex;
		};
