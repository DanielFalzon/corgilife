...
switch _mode do {
	...
	case "add": {
		...
		if ((_cartelSelection select 0) != group player getVariable ["gang_name",""]) then {
			_cartelMultiplier = 1.05;
		};
		
		MAVV(Total) = MAVV(Total) + (_price * _cartelMultiplier);
	...
	case "complete": {
		...
		// Declarations for cartel system variables.
		private _cartelSelection = CL_capture_list select 0;
		...
		// Added an if statement to ensure that the player has enough money to pay for the tax and items in the cart.
		if (((_cartelSelection select 0) != group player getVariable ["gang_name",""]) && (MAVV(Total) * 1.05 > CASH)) exitWith {
			hint getText (MAVCFG >> "localization" >> "msgNotEnoughCash");
		};
		...
		// Where the funds are taken off the player for the transaction to take place.

		if ((_cartelSelection select 0) != group player getVariable ["gang_name",""]) then {
			_oldPrice = _cash;
			_taxed = true;
			_cash = round(_cash * 1.05);
			_taxedAmount = round(_cash - _oldPrice);
		} else {
			_cartelOwner = true;
		};
