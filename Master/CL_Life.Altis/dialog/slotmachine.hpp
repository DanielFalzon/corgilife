class CL_slotmachine {
	name = "CL_slotmachine";
	idd = 38205;
	fadein=0;
	duration = 99999999999;
	fadeout=0;
	movingEnable = 0;
	onLoad="uiNamespace setVariable ['CL_slotmachine',_this select 0]";
	objects[]={};

	class controlsBackground {
		class background : CL_RscPicture {
			idc = -1;
			text = "textures\Slots\SlotMachine\slotbg.jpg";
			x = safeZoneX+(safezoneW * 0.5)-0.5; y = safeZoneY+(safezoneH * 0.5)-0.5;
			w = 1; h = 1;
		};
		
		class ProgressText : CL_RscText {
			idc = 38206;
			text = "Lucky Diamonds";
			x = safeZoneX+(safezoneW * 0.5)-0.1;
			y = safeZoneY+(safezoneH * 0.5)-0.5;
			w = 0.65; h = 0.1;
		};
		
		class roll1 : CL_RscPicture {
			idc = 38207;
			text = "textures\Slots\SlotMachine\bar.jpg";
			x = safeZoneX+(safezoneW * 0.25) + 0.380; 
			y = safeZoneY+(safezoneH * 0.5)-0.09; // 0.11 to 0.15
			w = 0.13; h = 0.25;
		};
		
		class roll2 : CL_RscPicture {
			idc = 38208;
			text = "textures\Slots\SlotMachine\bar.jpg";
			x = safeZoneX+(safezoneW * 0.25) + 0.520; 
			y = safeZoneY+(safezoneH * 0.5)-0.09;
			w = 0.13; h = 0.25;
		};
		
		class roll3 : CL_RscPicture {
			idc = 38209;
			text = "textures\Slots\SlotMachine\bar.jpg";
			x = safeZoneX+(safezoneW * 0.25) + 0.660; 
			y = safeZoneY+(safezoneH * 0.5)-0.09;
			w = 0.13; h = 0.25;
		};
	};
};