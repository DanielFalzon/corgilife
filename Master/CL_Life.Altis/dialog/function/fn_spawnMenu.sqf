...
if (CL_is_arrested) exitWith {
	[] call CL_fnc_respawned;
};

if (CL_respawned) then {
	[] call CL_fnc_respawned;
};
...
if (!(createDialog "CL_spawn_selection")) exitWith {[] call CL_fnc_spawnMenu;};
(findDisplay 38500) displaySetEventHandler ["keyDown","_this call CL_fnc_displayHandler"];

private _spCfg = [playerSide] call CL_fnc_spawnPointCfg;
...
[((findDisplay 38500) displayCtrl 38502),1,0.1,getMarkerPos (_sp select 0)] call CL_fnc_setMapPosition;
CL_spawn_point = _sp;
