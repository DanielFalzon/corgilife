class CL_Admin_Compensate {
	idd = 9920;
	name= "CL_Admin_Compensate_give";
	movingEnable = 0;
	enableSimulation = 1;
	...
	class controls {
		...
		class AdminCloseComp: CL_RscButtonMenu {
			idc = -1;
		...
		class AdminCompensVer: CL_RscButtonMenu {
			idc = -1;
			text = "$STR_Admin_Compensate";
			colorBackground[] = {"(profilenamespace getvariable ['GUI_BCG_RGB_R',0.3843])", "(profilenamespace getvariable ['GUI_BCG_RGB_G',0.7019])", "(profilenamespace getvariable ['GUI_BCG_RGB_B',0.8862])", 0.5};
			onButtonClick = "[] spawn CL_fnc_adminCompensate;";
			x = 0.27;
			y = 0.40;
			w = (6.25 / 40);
			h = (1 / 25);
		...
		class AdminCompensTex: CL_RscEdit {
			idc = 9922;
			text = "";
