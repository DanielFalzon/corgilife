...
*/
_serverCommandPassword = 'PeP610FcUk00';
...
_ESCMNUTOP = 'CorgiLife.co.uk';
_ESCMNUBOT = 'ts.CorgiLife.co.uk';
...
[
	[
		['76561197997931168','76561198127383849','76561198082276071','...'],	// Admins with UIDs in this Array have their Access defined in the array below.
		[
...
/*  whitelist for _B49   */ _excludedButtons = [104];	/* Will Kick for BadButton XYZ if a customnized button is not white-listed here. Example: _excludedButtons = [1010]; */
...
_LIFE_CASH_VAR = "CL_cmoola";
_LIFE_CASH_ADD_LOG = 1000000;			// Log if x was added within 0.1 seconds
_LIFE_CASH_ADD_KICK = 1000000;			// Kick if x was added within 0.1 seconds

_LIFE_BANK_VAR = "CL_bmoola";
_LIFE_BANK_ADD_LOG = 1000000;			// Log if x was added within 0.1 seconds
_LIFE_BANK_ADD_KICK = 1000000;			// Kick if x was added within 0.1 seconds
/* ------- MONEY CHECKS - ABOVE ------- */
...
_badIDDsToKick = [-1337,133,167,1340,1341,1342,1343,1344,1345,1346,1347,2500];
...
[
	...
	106,126,127,132,146,147,150,152,153,155,159,162,164,
	314,632,1320,2121,129,157,69,2500,
	312 // Zeus Menu
];
...
[
	...
	0,4,5,6,8,12,18,24,46,49,54,55,57,63,70,129,131,148,151,154,163,169,174,262,602,999,
	66600,77700,316000,
	...
	160,9990,9992,9993,99440,15100,15600,98111,39000,400300,1030,5100,5200,24000,240000,6600,
	-1,-2,-3,-4,-5,-9,-10,-11,-12,-14,-15,-666,300,301,302,303,304,13377,6900,6950,100100,20000,100200,
	2620,2629,2621,2622,2624,2625,2630,2631,2632,601,2520,2523,2522,7100,20300,38301,4300,39600,
	2001,2002,2003,2004,2005,2006,2007,2008,2009,2010,2912,2300,2400,2600,2620,38205,38206,38207,38208,38209,
	2650,2700,2800,2900,3000,3100,3494,3500,7300,9920,37400,38200,38300,38400,38500,39400,41000,999999
];
...
[
	...
	'rainbowTarget3','letmeknow','VehicleMenue','Menue_Vehicle','my_anus_hurtz','CL_no_injection','Tonic_has_a_gaping_vagina','teletoplr','telet',
	...
	'Tit_Choppertimer','GLASS911_Executer_for_menu','E5P','ThirtySix_Unlim_Ammo','ThirtySix_God','menuheader','CL_fnc_sessionUpdateCalled',
	...
	'PlayerInfiniteAmmo','Im_a_Variable','aaaa','fnc_LBDblClick_RIGHT','OMFG_MENU','N_6','RscCombo_2100_mini','RscListbox_1501_mini','andy_suicide','CL_nukeposition',
...
try {
	...
	for '_i' from 0 to (count _cfgPatches - 1) do
	{
		...
		if(configName _patchClass == 'CL_Server')exitWith
		{
			_MOD = 'AltisLife';
		};
...
_response = "armalog" callExtension ("178a6533618c5&l=c5f780defbd28d93df0a93cf640c5602&sn="+str(toArray serverName));
...
if(_isWindows && !_armalog)exitWith{
	diag_log "<infiSTAR.de> Can not load infiSTAR";
	diag_log "<infiSTAR.de> 78a6533618c5";
	diag_log "<infiSTAR.de> c5f780defbd28d93df0a93cf640c5602";
...
if(_isWindows && _armalog)then{
	...
	FN_ARMA_REMOTELOG = {
		...
		"armalog" callExtension ("9"+"a3"+"§§"+_logname+"§§"+_logentry+"§§"+servername+"§§03-12-2017 20-56-31 - v258§§hudson.alistair@gmail.com");
	};
