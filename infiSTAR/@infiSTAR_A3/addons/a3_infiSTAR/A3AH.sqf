...
/* *********************************www.infiSTAR.de********************************* */
diag_log '<infiSTAR.de> VERSION: infiSTAR.de 03-12-2017 20-56-31 (v258)';
comment 'Antihack & AdminTools - Christian Lorenzen - www.infiSTAR.de - License: (CC)';
UPDATEEMAIL='hudson.alistair@gmail.com';
INFISTARVERSION='03-12-2017 20-56-31 - v258';
...
if(!_return)exitWith
{
	diag_log format['<infiSTAR.de> %1 - serverCommandPassword NOT SET!    hudson.alistair@gmail.com - 03-12-2017 20-56-31 - v258 - %2 - %3',time,serverName,productVersion];
...
{
	diag_log format['<infiSTAR.de> %1 - infiSTAR_AdminMenu.hpp is not in your MPMission or you forgot to #include it within your description.ext   [hudson.alistair@gmail.com - 03-12-2017 20-56-31 - v258 - %2 - %3]',_t,serverName,productVersion];
};
_test = [0,{}] execFSM 'call.fsm';
_test = [0,{}] execFSM 'call.fsm';
if(_test isEqualTo 0)exitWith
{
	_log = format['<infiSTAR.de> %1 - call.fsm missing in your MPmission!    hudson.alistair@gmail.com - 03-12-2017 20-56-31 - v258 - %2 - %3',_t,serverName,productVersion];
...
FNC_A3_CUSTOMLOG = compileFinal "
	...
	diag_log format['<infiSTAR.de>%1| %2   [03-12-2017 20-56-31 - v258]',_logname,_logentry];
...
_fnc_RandomGen =
{
	_fnc_actualGen = {
		...
		_arr = ['a','c','5','f','7','8','0','d','e','f','b','d','2','8','d','9','3','d','f','0','a','9','3','c','f','6','4','0','c','5','6','0','2'];
...
_blacklistedVariables append [
	...
	'kick_admins','dasmokeon','hovering','r_kelly_be_flying','slx_xeh_bwc_init_compile','vincelol_altislife','CL_fnc_byassskaroah','ah_fnc_mp','jayre','fn_newsbanner','hack_news','trollfuncs',
	...
	'targetfuncs2','CL_fnc_antifreeeeze','ly_keyforward','ty_re_onetime','CL_fnc_xaaxaa','mein1','goddamnvehiclesxd','mystic_fnc_esp_distance','esp_id_setter','dummymen','whipbut','userfuncs',
...
if(_option isEqualTo 15)exitWith
{
	...
	if(_value)then
	{
		...
		[_array select 1] remoteExec ['CL_fnc_restrain',_unit];
	}
	else
	{
		_unit setVariable['restrained',_value,true];
...
_log call FNC_A3_CONNECTLOG;
_m = missionNameSpace getVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),''];_m = _m + '&cid=4919';missionNameSpace setVariable [('UPD'+'A'+'TE'+'E'+'M'+'AI'+'L'),_m];
...
"; }; _A3AHstring = _A3AHstring + "
	...
	if!(_admin)then
	{
		...
		_badclassesban =
		[
			'CL_Server','devcon','pooploop','rscrazzler','rsclistboxvg','vgdialog','niggers_die','ballers_die','grimbae','nss_admin_console','sspcm','balca_debug_tool','tonis_admin_menu_main',
			'oks_die'
		];
		_badclasseskick =
		[
	...
	if(_isNormal)then {
		...
		[_name,_puid] spawn {
			...
			_addCaseHDMGold = -1;
			_addCaseMEHold = -1;
			if(isNil 'CL_fnc_keyHandler')then{CL_fnc_keyHandler = {false};}else{if(typeName CL_fnc_keyHandler != 'CODE')then{CL_fnc_keyHandler = {false};};};
			...
			_actionTitle = toLower _actionTitle;
			{
				...
				"; if(_RDK)then{ _A3AHstring = _A3AHstring + "
					...
					"; if(_MOD == 'AltisLife')then{ _A3AHstring = _A3AHstring + "
						...
						_kdID = (findDisplay 46) displayAddEventHandler ['KeyDown','_this call CL_fnc_keyHandler;'];
					"; }; _A3AHstring = _A3AHstring + "
					"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
						_shouldbe = _shouldbe + 1;
					...
					if(alive player)then
					{
						...
						"; if(_MOD == 'AltisLife')then{ _A3AHstring = _A3AHstring + "
							...
							"; if(_RKH)then{ _A3AHstring = _A3AHstring + "
								...
								player addEventHandler ['Killed', {_this spawn CL_fnc_onPlayerKilled}];
							"; }; _A3AHstring = _A3AHstring + "
							"; if(_RHD)then{ _A3AHstring = _A3AHstring + "
								player removeAllEventhandlers 'HandleDamage';
								player addEventHandler ['HandleDamage',{_this call CL_fnc_HandleDamage;}];
							"; }; _A3AHstring = _A3AHstring + "
							player addEventHandler ['Take',{_this call CL_fnc_onTakeItem}];
...
"; }; _A3AHstring = _A3AHstring + "
		...
		[_name,_puid,_admins] spawn {
			...
			"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
				...
				_rnd1 = round(random 99999);CL_cmoola = _rnd1;
				_rnd2 = round(random 99999);CL_adminlevel = _rnd2;
				_rnd3 = round(random 99999);CL_coplevel = _rnd3;
				_rnd4 = round(random 99999);CL_fnc_MPexec = _rnd4;
				_rnd5 = round(random 99999);CL_bmoola = _rnd5;
			"; }; _A3AHstring = _A3AHstring + "
			_caeM1 = 0;
			_caeM2 = 0;
			...
			while{1==1}do
			{
				...
				"; if(_MOD == 'Epoch')then{ _A3AHstring = _A3AHstring + "
					if!(_rnd1 isEqualTo CL_cmoola)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Money Hack: [%1] should be [%2]',CL_cmoola,_rnd1])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd2 isEqualTo CL_adminlevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Adminlevel Hack: [%1] should be [%2]',CL_adminlevel,_rnd2])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd3 isEqualTo CL_coplevel)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Coplevel Hack: [%1] should be [%2]',CL_coplevel,_rnd3])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd4 isEqualTo CL_fnc_MPexec)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: CL_fnc_MPexec [%1] should be [%2]',CL_fnc_MPexec,_rnd4])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
					if!(_rnd5 isEqualTo CL_bmoola)then
					{
						[_name,_puid,'BAN',toArray(format['Altis Life Hack: CL_bmoola [%1] should be [%2]',CL_bmoola,_rnd5])] call "+_AHKickLog+";[] call "+_AHKickOFF+";
					};
				"; }; _A3AHstring = _A3AHstring + "
				"+_ninetwo+" = true;
	...
	_hours = floor(serverTime / 60 / 60);_value = ((serverTime / 60 / 60) - _hours);if(_value == 0)then{_value = 0.0001;};_minutes = round(_value * 60);_seconds = '9838x2';
	_devLog = format['<infiSTAR.de> %1 VERSION: infiSTAR.de 03-12-2017 20-56-31 (v258) - server running: %2:%3:%4',VERSIONCHECKRESULT,_hours,_minutes,_seconds];diag_log _devLog;
...
_res = [VERSION_CHECK_URL_FULL+'&id=78a6533618c5&li=c5f780defbd28d93df0a93cf640c5602&pr=A3'] call FN_ARMA_FETCHDATA;
	...
	"; if(_CGM)then{ _A3AHstring = _A3AHstring + "
		...
		};
		{
			if!(_x in "+_MCS+")then
			{
				...
				if(_do)then
				{
					if(_lowMarker in ['gefmarker','deinvadder','swagmarker','dmcmarking','CL_mpPacket_send'])then
					{
