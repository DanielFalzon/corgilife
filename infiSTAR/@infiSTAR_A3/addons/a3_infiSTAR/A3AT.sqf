...
VERSION_DATE_IS = '03-12-2017 20-56-31';
infiSTAR_MAIN_CODE = "
...
fnc_LBDblClick_RIGHT = {
	...
	{
		if(_click == 'Give All Licenses')then{{missionnamespace setVariable[(_x select 0),true];} foreach CL_licenses;};
		...
		FNC_VIRTUAL_ITEMSTHREAD = {
			...
			(findDisplay MAIN_DISPLAY_ID displayCtrl 65466) ctrlSetEventHandler['ButtonClick','
				...
				if(_msg != '''')then
				{
					...
					[''[true,''+str _click+'',''+str _value+''] call CL_fnc_handleInv;'',SELECTED_TARGET_PLAYER] call admin_d0_target;
...
fnc_Altislife_money = {
	...
	if(_state == 'cash')then
	{
		CL_cmoola = CL_cmoola + _amount;
		_log = format['Cash %1K - %2K has been added.',CL_cmoola/1000,_amount/1000];
		_log call FN_SHOW_LOG;
	};
	if(_state == 'bank')then
	{
		CL_bmoola = CL_bmoola + _amount;
		_log = format['Bank %1K - %2K has been added.',CL_bmoola/1000,_amount/1000];
		_log call FN_SHOW_LOG;
	};
};
admin_showinfo = {
...
fnc_ReviveTarget = {
	...
	if(infiSTAR_MOD == 'AltisLife')then
	{
		...
		[profileName] remoteExec ['CL_fnc_revived',_target];
	}
	else
	{
...
infiSTAR_A3Heal = {
	if(!isNil 'CL_cmoola')then{CL_thirst = 100;CL_hunger = 100;};
	if(infiSTAR_MOD == 'Epoch')then{
	...
	if(_veh isKindOf 'AllVehicles')exitWith{
		if((player distance _veh < 12) || ((_veh == vehicle player) && (vehicle player != player)))then
		{
			...
			if(_locked isEqualTo 2)then
			{
				...
				{
					_veh lock 0;
					[_veh,0] remoteExec ['CL_fnc_lockVehicle',_veh];
					[_veh,'unlockCarSound'] remoteExec ['CL_fnc_say3D',0];
				}
				else
				{
					_veh lock false;
			...
			else
			{
				...
				{
					_veh lock 2;
					[_veh,2] remoteExec ['CL_fnc_lockVehicle',_veh];
					[_veh,'lockCarSound'] remoteExec ['CL_fnc_say3D',0];
				}
				else
				{
					_veh lock true;
...
infiSTAR_A3Invulnerability = {
	fnc_A3_stopGod =
	{
		...
		if((infiSTAR_MOD == 'AltisLife') && (!isNil 'CL_fnc_HandleDamage'))then
		{
		...
		if(!isNil 'oCL_fnc_tazed')then{CL_fnc_tazed = oCL_fnc_tazed;};
	};
	...
	if(infiSTAR_A3GodRun == 0)then
	{
		...
		if(!isNil 'CL_fnc_tazed')then
		{
			if(isNil 'oCL_fnc_tazed')then
			{
				oCL_fnc_tazed = CL_fnc_tazed;
			};
			CL_fnc_tazed = {};
		};
